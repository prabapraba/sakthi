...
- Bug #5424: `Html::addCssStyle()` wasn't correctly setting style passed in array (raja, samdark)
...
- Bug #3194: Date formatter works only for timestamps in the year range 1970 to 2038 (raja)
...
- Bug #3756: Fix number formatting error for `\yii\base\Formatter` by converting strings to float (raja)
...
- Enh #3196: Masked input upgraded to use jquery.inputmask plugin with more features. (raja)
...
- Enh #3472: Added configurable option to encode spaces in dropDownLists and listBoxes (raja)
...
- Enh #5117: Added `beforeFilter` and `afterFilter` JS events to `GridView` (raja)
...
- Enh #2392: Added `addCssStyle()`, `removeCssStyle()`, `cssStyleFromArray()` and `cssStyleToArray()` to `Html` (qiangxue, raja, Alex-Code)
